-- ============================================
-- Recreate buyers/sellers tables with proper architecture
-- users: basic auth info only
-- buyers: buyer activity data
-- sellers: seller activity data
-- One user can have both buyer and seller records
-- ============================================

-- ============================================
-- 1. Create buyers table (구매자 활동 데이터)
-- ============================================
...
-- ============================================
-- 2. Create sellers table (판매자 활동 데이터)
-- ============================================
...
FROM public.users
WHERE NOT EXISTS (SELECT 1 FROM public.buyers WHERE user_id = users.id)
ON CONFLICT (user_id) DO NOTHING;

-- user_type이 'seller'인 users를 sellers로
INSERT INTO public.sellers (user_id, created_at, updated_at)
SELECT id, created_at, updated_at
FROM public.users
WHERE user_type = 'seller'
AND NOT EXISTS (SELECT 1 FROM public.sellers WHERE user_id = users.id)
ON CONFLICT (user_id) DO NOTHING;

-- ============================================
-- 4. Remove user_type from users table
-- ============================================
