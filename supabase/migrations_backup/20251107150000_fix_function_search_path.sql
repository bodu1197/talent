...
COMMENT ON FUNCTION public.set_visited_date IS 'Records or updates category visit timestamp for a user with secure search_path';
COMMENT ON FUNCTION public.cleanup_old_category_visits IS 'Removes category visits older than 90 days with secure search_path';
COMMENT ON FUNCTION public.get_recent_category_visits IS 'Gets recent category visits for a user with secure search_path';
COMMENT ON FUNCTION public.get_random_ai_services IS 'Returns random AI services with secure search_path';
COMMENT ON FUNCTION public.update_chat_room_last_message IS 'Trigger function to update chat room last message with secure search_path';
COMMENT ON FUNCTION public.get_seller_id IS 'Gets seller ID from user ID with secure search_path';
COMMENT ON FUNCTION public.get_buyer_id IS 'Gets buyer ID from user ID with secure search_path';
