-- ========================================
-- AI 서비스 카테고리 재구성 (수정본)
-- service_categories 테이블의 category_id는 slug(문자열)를 저장함
-- ========================================

-- 1단계: AI 카테고리를 사용하는 service_categories 연결 삭제
-- (category_id가 slug이므로 slug로 삭제)

-- AI 서비스의 모든 하위 카테고리 slug 목록 조회 및 삭제
DELETE FROM service_categories
WHERE category_id IN (
  -- 2단계 카테고리 slug
  SELECT slug FROM categories WHERE parent_id = (SELECT id FROM categories WHERE slug = 'ai-services')
  UNION
  -- 3단계 카테고리 slug
  SELECT c3.slug FROM categories c3
  WHERE c3.parent_id IN (
    SELECT c2.id FROM categories c2 WHERE c2.parent_id = (SELECT id FROM categories WHERE slug = 'ai-services')
  )
);
...
BEGIN
  ...
  -- 5. AI 프롬프트
  INSERT INTO categories (name, slug, icon, description, parent_id, level, is_ai, is_active, display_order)
  VALUES ('AI 프롬프트', 'ai-prompt-new', 'terminal', 'AI 프롬프트 제작 및 판매', ai_services_id, 2, true, true, 5)
...
-- 삭제된 service_categories 확인
SELECT COUNT(*) as "남은_AI_서비스_연결"
FROM service_categories
WHERE category_id LIKE 'ai-%';
