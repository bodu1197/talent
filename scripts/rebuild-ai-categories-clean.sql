-- ========================================
-- AI 서비스 카테고리 완전 초기화 후 재구성
-- ========================================

-- 1단계: AI 카테고리를 사용하는 service_categories 연결 모두 삭제
DELETE FROM service_categories
WHERE category_id IN (
  SELECT slug FROM categories
  WHERE slug LIKE 'ai-%' OR slug LIKE '%ai-%' OR slug LIKE 'design-prompt' OR slug LIKE 'music-video-prompt' OR slug LIKE 'content-prompt'
);

-- 2단계: 기존 AI 관련 카테고리 완전 삭제
-- 새로 생성할 slug들도 포함해서 삭제
DELETE FROM categories
WHERE slug IN (
  -- 새로운 2단계
  'ai-design',
  'ai-video-photo-audio',
  'ai-development-automation',
  'ai-marketing-writing',
  'ai-prompt-new',
  'ai-utilization-monetization',
  -- 새로운 3단계
  'ai-realistic-model',
  'ai-illustration',
  'ai-ad-material-design',
  'ai-ad-video',
  'ai-product-photo',
  'ai-dubbing-narration',
  'ai-system-service',
...
-- 3단계: 기존 AI 서비스 하위 카테고리 모두 삭제 (혹시 남은 것들)
-- 3단계 카테고리 삭제
