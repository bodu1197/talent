...
-- 1. 기존 생활 서비스 관련 카테고리 조회 및 삭제를 위한 임시 변수
DO $$
DECLARE
  life_service_uuid UUID;
BEGIN
  -- 생활 서비스 ID 조회
  SELECT id INTO life_service_uuid FROM categories WHERE slug = 'life-service';

  IF life_service_uuid IS NOT NULL THEN
    -- 3차 카테고리 삭제
    DELETE FROM categories
    WHERE parent_id IN (
      SELECT id FROM categories WHERE parent_id = life_service_uuid
    );

    -- 2차 카테고리 삭제
    DELETE FROM categories WHERE parent_id = life_service_uuid;

    -- 1차 카테고리 삭제
    DELETE FROM categories WHERE id = life_service_uuid;
  END IF;
END $$;
