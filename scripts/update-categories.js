...
async function updateLifeServiceCategories() {
  ...
  try {
    // 1. 기존 생활 서비스 3차 카테고리 삭제
    console.log('1️⃣ 기존 3차 카테고리 삭제 중...')
    const { data: level2Categories } = await supabase
      .from('categories')
      .select('id')
      .eq('parent_id', 'life-service')

    if (level2Categories && level2Categories.length > 0) {
      const level2Ids = level2Categories.map(c => c.id)
      const { error: deleteLevel3Error } = await supabase
        .from('categories')
        .delete()
        .in('parent_id', level2Ids)

      if (deleteLevel3Error) throw deleteLevel3Error
      console.log('   ✅ 3차 카테고리 삭제 완료')
    }

    // 2. 기존 생활 서비스 2차 카테고리 삭제
    console.log('2️⃣ 기존 2차 카테고리 삭제 중...')
    const { error: deleteLevel2Error } = await supabase
      .from('categories')
      .delete()
      .eq('parent_id', 'life-service')

    if (deleteLevel2Error) throw deleteLevel2Error
    console.log('   ✅ 2차 카테고리 삭제 완료')

    // 3. 기존 생활 서비스 1차 카테고리 삭제
    console.log('3️⃣ 기존 1차 카테고리 삭제 중...')
    const { error: deleteLevel1Error } = await supabase
      .from('categories')
      .delete()
      .eq('id', 'life-service')

    if (deleteLevel1Error) throw deleteLevel1Error
    console.log('   ✅ 1차 카테고리 삭제 완료\n')

    // 4. 새로운 1차 카테고리 생성
    console.log('4️⃣ 새로운 1차 카테고리 생성 중...')
    const { error: insertLevel1Error } = await supabase
      .from('categories')
      .insert({
        id: 'life-service',
        name: '생활 서비스',
    ...
    if (insertLevel1Error) throw insertLevel1Error
    console.log('   ✅ 1차 카테고리 생성 완료')

    // 5. 새로운 2차 카테고리 생성 (4개)
    console.log('5️⃣ 새로운 2차 카테고리 생성 중...')
    const newLevel2Categories = [
    ...
    const { error: insertLevel2Error } = await supabase
      .from('categories')
      .insert(newLevel2Categories.map(cat => ({
        ...cat,
    ...
    if (insertLevel2Error) throw insertLevel2Error
    ...
    const { error: insertProfCleaningError } = await supabase
      .from('categories')
      .insert(professionalCleaningItems.map(cat => ({
        ...cat,
    ...
    if (insertProfCleaningError) throw insertProfCleaningError
    ...
    const { error: insertHomeHelperError } = await supabase
      .from('categories')
      .insert(homeHelperItems.map(cat => ({
        ...cat,
    ...
    if (insertHomeHelperError) throw insertHomeHelperError
    ...
    const { error: insertHomeRepairError } = await supabase
      .from('categories')
      .insert(homeRepairItems.map(cat => ({
        ...cat,
    ...
    if (insertHomeRepairError) throw insertHomeRepairError
    ...
    const { error: insertMovingError } = await supabase
      .from('categories')
      .insert(movingServiceItems.map(cat => ({
        ...cat,
    ...
    if (insertMovingError) throw insertMovingError
