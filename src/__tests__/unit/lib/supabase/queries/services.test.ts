...
describe('Supabase Queries - Services', () => {
      ...
      const mockData = [
        {
          ...
          service_categories: [{ category: { id: 'cat-1', name: 'Category 1' } }],
        },
      ];
  ...
  describe('getServiceById', () => {
    it('should return service with seller and category information', async () => {
      const mockService = {
        ...
        service_categories: [{ category: { id: 'cat-1', name: 'Category 1', slug: 'cat-1' } }],
      };
  ...
  describe('getSellerServicesCount', () => {
    ...
    it('should throw error on query failure', async () => {
      ...
      await expect(getSellerServicesCount('seller-123', 'active')).rejects.toEqual(mockError);
    });
  });
  ...
  describe('getServicesByCategory', () => {
    it('should return services for a 3rd level category', async () => {
      ...
      const mockServiceLinks = [{ service_id: 'service-1' }, { service_id: 'service-2' }];
      const mockServices = [
        {
          id: 'service-1',
          title: 'Service 1',
  ...
  describe('getActiveServices', () => {
    it('should return active services excluding AI category', async () => {
      ...
      const mockQueries = {
        categories: {
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockReturnThis(),
          maybeSingle: vi.fn().mockResolvedValue({ data: mockAiCategory, error: null }),
        },
        service_categories: {
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockResolvedValue({ data: mockAiServiceLinks, error: null }),
        },
        services: {
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockReturnThis(),
    ...
    it('should add rating and review_count to services', async () => {
      ...
      expect((services[0] as { orders_count: number }).orders_count).toBe(10);
    });
  });
});
