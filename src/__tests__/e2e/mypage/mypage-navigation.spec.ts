...
test.describe('마이페이지 네비게이션', () => {
  ...
  test('마이페이지 접근 및 기본 정보 표시', async ({ page }) => {
    ...
    const hasUserInfo =
      (await page
        .locator('[data-testid="user-profile"], .user-info, text=/프로필|Profile/')
        .count()) > 0;
  ...
  test('마이페이지 탭 네비게이션', async ({ page }) => {
    ...
    // 구매자 탭이 있는지 확인
    const buyerTab = page
      .locator('a[href*="/mypage/buyer"], button:has-text("구매"), [data-testid="buyer-tab"]')
      .first();
    ...
    if ((await buyerTab.count()) > 0) {
      ...
      // URL 변경 또는 콘텐츠 변경 확인
      const hasBuyerContent =
        (await page.locator('[data-testid="buyer-dashboard"], text=/구매|주문/').count()) > 0;
      expect(hasBuyerContent || page.url().includes('buyer')).toBeTruthy();
    }

    // 전문가 탭이 있는지 확인
    const sellerTab = page
      .locator('a[href*="/mypage/seller"], button:has-text("전문가"), [data-testid="seller-tab"]')
      .first();
    ...
    if ((await sellerTab.count()) > 0) {
      ...
      const hasSellerContent =
        (await page.locator('[data-testid="seller-dashboard"], text=/전문가|서비스/').count()) > 0;
  ...
  test('F5 새로고침 후에도 정상 작동', async ({ page }) => {
    ...
    const hasLoadingSpinner =
      (await page.locator('[data-testid="loading"], .animate-spin, text=/로딩|Loading/').count()) >
      0;

    // 로딩 스피너가 없어야 함 (또는 있더라도 곧 사라져야 함)
    if (hasLoadingSpinner) {
      await expect(page.locator('[data-testid="loading"], .animate-spin')).not.toBeVisible({
        timeout: 3000,
      });
    }
  });
  ...
  test('로그아웃 기능', async ({ page }) => {
    ...
    const logoutButton = page
      .locator('button:has-text("로그아웃"), a:has-text("로그아웃"), [data-testid="logout"]')
      .first();
