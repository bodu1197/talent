        ...
        <div className="grid grid-cols-5 gap-2">
          {PRESET_COLORS.map((color) => (
            <button
              ...
              type="button"
              onClick={() =>
                setTextStyle((prev) => ({ ...prev, color: color.value }))
              }
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <button
              ...
              type="button"
              onClick={() =>
                setTextStyle((prev) => ({ ...prev, fontWeight: weight.value }))
              }
              className={`px-3 py-2 border rounded-lg text-sm transition-all ${
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-800">
              편집이 완료되면 반드시 아래 <strong>"썸네일 생성하기"</strong>{" "}
              버튼을 눌러주세요. 버튼을 누르지 않으면 썸네일이 저장되지
              않습니다.
            </p>
          </div>
        </div>
