...
export const BACKGROUND_TEMPLATES: GradientTemplate[] = [
  // 비즈니스/전문가 느낌
  { id: 'bg-1', name: '프로페셔널 블루', type: 'linear', colors: ['#0f3460', '#1a5490'], angle: 135 },
  { id: 'bg-2', name: '다크 네이비', type: 'linear', colors: ['#2c3e50', '#3498db'], angle: 45 },
  { id: 'bg-3', name: '비즈니스 그레이', type: 'linear', colors: ['#434343', '#000000'], angle: 180 },

  // 크리에이티브/디자인
  { id: 'bg-4', name: '크리에이티브 퍼플', type: 'linear', colors: ['#667eea', '#764ba2'], angle: 135 },
...
export async function generateThumbnailWithText(
  ...
  // 실제 위치 계산
  const actualX = textOptions.x * width
  const actualY = textOptions.y * height

  ctx.fillText(textOptions.text, actualX, actualY)
