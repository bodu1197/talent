...
export async function getUserConversations(userId: string) {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      ...
      participant1:users!participant1_id(id, name, profile_image),
      participant2:users!participant2_id(id, name, profile_image),
      messages(
        id,
        content,
        created_at,
  ...
  return data.map(conv => {
    ...
    const unreadCount = isParticipant1 ? conv.participant1_unread_count : conv.participant2_unread_count
...
export async function markMessagesAsRead(conversationId: string, userId: string) {
  ...
  if (conversation) {
    ...
    const field = isParticipant1 ? 'participant1_unread_count' : 'participant2_unread_count'
