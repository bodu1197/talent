...
export default function DirectChatClient({ roomId, userId, isSeller, otherUser, service }: Props) {
  ...
  const markMessagesAsRead = async () => {
    try {
      ...
      console.error('[DirectChatClient] Mark messages as read error:', JSON.stringify(error, Object.getOwnPropertyNames(error), 2))
    }
  }
...
function PaymentRequestModal({ roomId, service, onClose }: { roomId: string, service: Service | null, onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Payment request error:', JSON.stringify(error, Object.getOwnPropertyNames(error), 2))
