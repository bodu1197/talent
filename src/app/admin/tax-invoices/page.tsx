    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="text-3xl font-bold text-gray-900">{stats.thisMonthAmount.toLocaleString()}원</div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="w-10 h-10 bg-purple-50 rounded-lg flex items-center justify-center">
              <i className="fas fa-won-sign text-purple-600"></i>
            </div>
          </div>
          <div className="text-3xl font-bold text-gray-900">{stats.totalSupplyAmount.toLocaleString()}원</div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">발행일 (시작)</label>
            <input
              type="date"
              value={dateFrom}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">발행일 (종료)</label>
            <input
              type="date"
              value={dateTo}
              ...
              {filteredInvoices.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center text-gray-500">
                    {searchTerm || dateFrom || dateTo ? '검색 결과가 없습니다' : '발행된 세금계산서가 없습니다'}
                  </td>
                </tr>
              ) : (
                filteredInvoices.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-gray-900">{invoice.buyer_company_name}</div>
                      <div className="text-xs text-gray-500">{invoice.buyer_business_number}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="text-sm text-gray-900">{invoice.supply_amount.toLocaleString()}원</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="text-sm text-gray-900">{invoice.tax_amount.toLocaleString()}원</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="text-sm font-bold text-gray-900">{invoice.total_amount.toLocaleString()}원</div>
                    </td>
