...
export default function AdminReviewsPage() {
  ...
  async function loadReviews() {
    try {
      ...
      setError(err instanceof Error ? err.message : '리뷰 목록을 불러오는데 실패했습니다')
    } finally {
      setLoading(false)
    }
  }
  ...
  const tabs = [
    ...
    { value: '5' as RatingFilter, label: '⭐️⭐️⭐️⭐️⭐️', count: ratingCounts[5] },
    { value: '4' as RatingFilter, label: '⭐️⭐️⭐️⭐️', count: ratingCounts[4] },
        ...
        <div className="flex items-center gap-8">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-1 mb-2">
              {[1, 2, 3, 4, 5].map((star) => (
                <i key={star} className={`fas fa-star ${star <= Math.floor(avgRating) ? 'text-yellow-400' : 'text-gray-300'} text-xl`}></i>
              ))}
            </div>
            <div className="text-sm text-gray-600">총 {reviews.length}개의 리뷰</div>
          </div>
