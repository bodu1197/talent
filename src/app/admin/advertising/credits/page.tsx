...
export default function AdminCreditsPage() {
  ...
  const fetchCreditDetails = async (sellerId: string) => {
    try {
      const response = await fetch(`/api/admin/advertising/credits?sellerId=${sellerId}`)
      if (response.ok) {
        const data = await response.json()
    ...
    <div className="space-y-8">
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-6">
            <div className="flex items-start justify-between">
              ...
              <div className="rounded-lg bg-blue-50 p-3">
                <i className="fas fa-coins text-[#0f3460] text-xl"></i>
              </div>
            </div>
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-12 text-center">
          <i className="fas fa-coins text-slate-400 text-5xl mb-4"></i>
              ...
              <div className="px-6 py-4 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setGrantForm({ ...grantForm, sellerId: e.target.value })}
                    placeholder="UUID"
                    required
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setGrantForm({ ...grantForm, promotionType: e.target.value })}
                    className="w-full px-3 py-2 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0f3460]"
                  >
                      ...
                      <tbody className="divide-y divide-slate-200">
                        {transactions.map((tx) => (
                          <tr key={tx.id} className="hover:bg-slate-50">
                            ...
                            <td className={`px-4 py-3 text-sm text-right font-medium ${getTransactionTypeColor(tx.transaction_type)}`}>
