...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error } = await supabaseAdmin.storage.from('portfolio').upload(filePath, buffer, {
    ...
    // 공개 URL 생성
    const { data: urlData } = supabaseAdmin.storage.from('portfolio').getPublicUrl(filePath);
