...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!serviceId || !categoryId) {
      return NextResponse.json({ error: 'serviceId and categoryId are required' }, { status: 400 });
    }
    ...
    // 노출 기록 생성
    const { error: impressionError } = await supabase.from('advertising_impressions').insert({
    ...
    if (impressionError) {
      logger.error('Failed to create impression:', impressionError);
      return NextResponse.json({ error: 'Failed to create impression' }, { status: 500 });
    }
    ...
    if (updateError) {
      logger.error('Failed to update subscription stats:', updateError);
