...
export async function GET(request: NextRequest) {
  try {
    ...
    const paymentsWithSellers = await Promise.all(
    ...
    const paymentsWithAdmins = await Promise.all(
      paymentsWithSellers.map(async (payment) => {
...
async function updateRelatedSubscriptions(
  ...
  const subscriptionIds = updatedPayments.map((p) => p.subscription_id).filter(Boolean);
