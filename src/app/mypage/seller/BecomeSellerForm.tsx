...
export default function BecomeSellerForm({ userId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (insertError) {
        setError('전문가 등록에 실패했습니다: ' + insertError.message);
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-md mx-auto">
        <div className="bg-white rounded-lg shadow-md p-8">
          <h1 className="text-base md:text-lg font-semibold text-gray-900 mb-2">전문가 되기</h1>
          <p className="text-sm md:text-base text-gray-600 mb-6">전문가명을 입력하세요</p>
