                  ...
                  {bankAccountVerification.isVerified && (
                    <div className="mt-2 bg-green-50 border border-green-200 rounded-lg p-2">
                      <p className="text-xs text-green-700">
                        ...
                        예금주:{' '}
                        {`${(bankAccountVerification.data?.holderName as string) ?? profile.account_holder ?? ''}`}
                      </p>
                    </div>
                  )}
                      ...
                      <div className="mt-2 bg-green-50 border border-green-200 rounded-lg p-3">
                        ...
                        <div className="text-xs text-green-700 space-y-1">
                          {businessVerification.data.businessName ? (
                            <p>상호: {`${businessVerification.data.businessName as string}`}</p>
                          ) : null}
                          {businessVerification.data.representativeName ? (
                            <p>
                              대표자: {`${businessVerification.data.representativeName as string}`}
                            </p>
                          ) : null}
                          <p>
                            상태:{' '}
                            {businessVerification.data.isActive
                              ? '정상 영업 중'
                              : `${(businessVerification.data.status as string) ?? '확인됨'}`}
                          </p>
                        </div>
                      </div>
                    )}
