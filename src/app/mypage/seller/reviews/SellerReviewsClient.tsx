          ...
          <div className="lg:col-span-1 bg-white rounded-lg border border-gray-200 p-6">
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-1 mb-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <i key={star} className={`fas fa-star ${parseFloat(avgRating) >= star ? 'text-yellow-400' : 'text-gray-300'}`}></i>
                ))}
              </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          <div className="flex gap-1">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <i key={star} className={`fas fa-star ${review.rating >= star ? 'text-yellow-400' : 'text-gray-300'}`}></i>
                            ))}
                          </div>
                          <span className="font-bold text-gray-900">{review.rating}.0</span>
                        </div>
                        <div className="text-sm text-gray-600">
                          {review.buyer?.name || '구매자'} • {new Date(review.created_at).toLocaleDateString('ko-KR')}
                        </div>
                      </div>
                      {!review.reply && (
                        <button
              ...
              {selectedReview && (
                <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                  <div className="flex gap-1 mb-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <i key={star} className={`fas fa-star ${selectedReview.rating >= star ? 'text-yellow-400' : 'text-gray-300'}`}></i>
                    ))}
                  </div>
